...
- 画面表示は「園名 / 法人名」の順で併記する
- 既存データでどちらかが欠けている場合は、入力済みの値を優先して表示する

## 認証・権限のルール

### 権限判定

**ルール**: 画面・APIの権限制御は`users.role`で判定する。

**基準**:
- `admin`: 管理画面およびユーザー管理APIを利用可能
- `user`: 通常画面のみ利用可能（ユーザー一覧の取得は可能）

## スプレッドシート取り込み（求職者同期）

### 取り込み対象と必須条件

**ルール**: `ID`（求職者ID）と`氏名`（氏名）がある行のみ取り込む。
...
**例外**:
- 既存レコードの`registered_at`（登録日）が空で、取り込み行に日付があれば登録日を補完する
- 既存IDで `氏名`（氏名）が不一致、または `（再登録）` が含まれる場合は、既存レコードを更新せず新IDを発行して追加する
- ただし、同一バッチ内やDB上ですでに同名（空白差分を無視）のレコードが別IDで存在する場合は重複追加せずスキップする

### 登録日（取り込み列）

**ルール**: 登録日は `日付` または `登録日` のどちらでも受け付ける。

### 取り込み上限

**ルール**: 1リクエストあたり最大3000行まで処理する（超過分は未処理）。

### 重複マージ時のデータ保全

**ルール**: 同一氏名の重複をマージする際は、`candidates`（求職者）本体の値を消さない。

**扱い**:
- 残すID側に値があればそれを優先する
- 残すID側が空で重複側に値がある項目は補完する
- `memo`（メモ）は片方が空なら補完、両方に値がある場合は重複を避けて結合する

### 担当者・媒体の解決

**ルール**: `担当者`（担当者名）と`媒体`（媒体名）は名称からマスタを引いてIDに変換する。
- 見つからない場合は `null` として登録する
...
**ルール**: 営業進捗テーブルには、以下いずれかに該当するメンバーを表示する

- 期間内に担当として登録された求職者が1人以上いる
- 期間内に status_history でステータス変更を実行した（`changed_by` に記録）
- 期間内に memos でメモを作成した（`created_by` に記録）

担当=0 の行は面談率などが `-` で表示される。詳細は`DASHBOARD_面接・成約の集計ロジック.md`を参照。
