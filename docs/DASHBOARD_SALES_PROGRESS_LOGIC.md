...
ダッシュボードの「営業進捗状況」は **同一列構成のテーブルが2つ** あります。

| テーブル | 担当・初回の母集団 | 面接・成約 |
|----------|-------------------|------------|
| 当月登録ベース | `registered_at` が**選択した暦月内**の求職者 | 上記母集団に属する人に限り、選択月内の面接・成約実績をカウント |
| 前月以前登録ベース | 画面内プルダウンで選んだ**過去月**に `registered_at` がある求職者（うち **担当・初回** は終了・ロスト系ステータスを除く） | 面接・成約も同じ過去月内の実績をカウント |

**前月以前テーブルの担当・初回から除外するステータス**: `クローズ（終了）` / `音信不通` / `内定辞退` / `連絡つかず（初回未接触）`（面接・成約列の集計には影響しません）。

面接・成約の定義（いつを「その月の面接」とみなすか等）は共通で、実装は `src/app/dashboard-summary/page.tsx` の `periodInterviewStatusCandidateIds` / `periodClosedStatusCandidateIds` を参照してください。  
**月次マージシート（`/api/metrics`）の数値で上書きする処理は行っていません**（母集団分割と両立しないため）。
