...
| **面談設定目標** | 目標設定値 | `settings`テーブル（未実装、現在はmock） |
| **面談設定数** | 期間内に面接を組んだ人数（1人複数件でも1カウント） | `interviews.created_at`が期間内のユニーク求職者数 |
| **対面談設定(%)** | (面談設定数 / 面談設定目標) × 100 | 計算値 |

### 重要なポイント

1. **成約額**: 期間内に設定（`created_at`）した成約の税抜売上合計
2. **面談設定数**: 期間内に面接を設定したユニークな求職者数（同じ求職者で複数面接を設定しても1カウント）
