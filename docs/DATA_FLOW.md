...
**目的**: 既存の求職者管理シートからデータを取り込み、Supabase に反映する。

**使用箇所**:
- Google Apps Script でシートの行を取得し、`POST /api/sync/candidates` に送信

**データの性質**:
- **外部データ**: シートの内容が正とみなされる
- **一括取り込み**: 1回の実行で最大3000行まで処理
- **既存IDは保全優先**: 既存IDは原則スキップし、`registered_at`（登録日）や連絡先は空欄のみ補完
- **IDずれ対策**: 既存IDで氏名不一致/再登録の場合のみ新IDを発行して追加（同名重複は抑止）
...
### 6. API Route Handlers（サーバー側API）

**目的**: 認証情報の取得とユーザー管理をサーバー側で提供する。

**使用箇所**:
- `GET /api/auth/me`: セッション検証とユーザー情報の取得
- `GET /api/users`: ユーザー一覧（認証済みなら取得可能）
- `POST /api/users`: ユーザー追加（管理者のみ）
- `POST /api/sync/candidates`: スプレッドシート取り込み（新規のみ追加）
...
スプレッドシート（連絡先一覧）
  ↓
Google Apps Script / API で行を抽出（最大3000行を処理）
  ↓
POST /api/sync/candidates
  ↓
Supabase（candidates）へ同期（新規追加 + 空欄補完）
  ↓
既存IDは空欄項目のみ補完、氏名不一致/再登録は新ID追加
  ↓
必要時のみ重複マージ（候補者本体の値を保全して統合）
```
