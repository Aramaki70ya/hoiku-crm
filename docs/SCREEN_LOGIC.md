...
- 期間が変更されても、データソース（`mock-data.ts`）は変わらない（フィルタリングは行わない）
...
- 期間選択でデータをフィルタリングするかどうか: **現時点ではフィルタリングしない**（全期間のデータを表示）
- 実績の計算方法: **常に`mockCandidates`から計算**（保存された値を使用しない）
