...
**なぜ必要か**: 選択期間の売上・見込み・登録・初回ヒアリング実績を一目で確認するため

**データの扱い方**:
- **売上合計**: 選択期間内の`contracts.revenue_including_tax`（税込売上）を優先し、未設定なら`contracts.revenue_excluding_tax`（税抜売上）を合計
- **ヨミ数字合計**: **求職者ステータスが「面接確定済」「面接実施済（結果待ち）」「内定獲得（承諾確認中）」のいずれか**に限定し、当月ヨミの`projects.expected_amount`（見込み金額）をA/B/Cヨミごとに集計した合計・内訳を表示（下段の担当者別ヨミテーブルとは対象ステータスが異なる）
- **新規登録数**: 選択期間内に`candidates.registered_at`（登録日時）が含まれる求職者数
- **初回ヒアリング数**: 選択期間内に登録された求職者のうち、初回未接触系ステータス以外の人数

**UIの動作**:
- 4つの実績カードを横並びで表示
- ヨミ数字は合計に加えて、A/B/Cの内訳も確認できるように表示（※サマリーカードのみ面接フェーズ3ステータスに限定）

**重要**: 主要実績は集計期間セレクトと連動して表示する（詳細は`DASHBOARD_REQUIREMENTS.md`を参照）
...
#### 6. 営業進捗テーブル（面接・成約内訳モーダル）

**なぜ必要か**: 担当者ごとの面接・成約の内訳を確認するため

**クリック機能**:
- **面接**の数字をクリック: その担当者の期間内面接対象者一覧をモーダルで表示（無効化可能）
- **成約**の数字をクリック: その担当者の期間内成約一覧をモーダルで表示（候補者名、承諾日、入職先、売上）

**活動メンバー表示**: 担当登録数が0でも、期間内にステータス変更またはメモ作成を行ったメンバーは営業進捗テーブルに表示される。詳細は`DASHBOARD_面接・成約の集計ロジック.md`を参照。

**ステータス変更タイミングベースの追加表**: 同じカード内の**表示順は「ステータス変更タイミングベース → 当月登録ベース → 前月以前登録ベース」**とする。`担当`（担当求職者数）と`初回`（初回接触済み人数）は当月登録ベースと同じ集計を使い、`面接`（選択月内に初めて面接確定済・実施済へ到達した人数）と`成約`（選択月内に成約した人数）だけ、登録月を問わない月内実績ベースに差し替える。分母と分子の母集団が異なるため、説明文で補助表であることを明記する。
...
- **面接先は「園名」「法人名」を分けて登録し、表示時は両方を併記する**
...
**なぜ必要か**: 担当者の売上・面接設定数などのKPIを確認するため

**データの扱い方**:
- `mockMemberStats`から該当ユーザーの統計情報を取得
- 売上予算、成約額、面接設定数、担当件数を表示
...
設定画面は、**月次の全体目標とメンバー別目標を設定する**ための画面です。
...
**なぜ必要か**: 月ごとに異なる目標を設定する必要があるため

**データの扱い方**:
- `selectedMonth` stateで管理（形式: `YYYY-MM`）
- 月を選択すると、全体目標とメンバー別目標の両方を該当月のデータで更新
- API経由で`monthly_targets`と`user_monthly_targets`を取得

**UIの動作**:
- 月選択セレクトボックスで年月を選択
- 選択した月の目標が即座に表示される
- 月が変更されると、両方の目標が更新される

#### 2. 全体目標設定カード

**なぜ必要か**: 組織全体の月次目標を管理するため

**データの扱い方**:
- `monthly_targets`テーブルから取得（API: `GET /api/targets?year_month=YYYY-MM`）
- 全体売上予算、KPI目標値（登録→初回率、初回→面接率、面接→成約率）、成約単価、面接設定目標を編集
- 保存時は`POST /api/targets`でupsert
...
**なぜ必要か**: 各コンサルタントの個別目標を管理するため

**データの扱い方**:
- `user_monthly_targets`テーブルから取得（API: `GET /api/user-targets?year_month=YYYY-MM`）
- `useUsers`フックでコンサルタント一覧を取得（role='user'のみ、管理者を除外）
- メンバーごとに売上予算、面接設定目標、成約件数目標を編集可能
- 保存時は全メンバー分を一括で`POST /api/user-targets`（配列形式）
...
**UIの動作**:
- テーブル形式で全メンバーの目標を一覧表示
- 各セルをクリックまたはインライン編集で値を変更
- 「すべて保存」ボタンで全メンバーの変更を一括保存

### 判断が必要な場面

- 月選択時の動作: **全体目標とメンバー別目標の両方を同時に更新**（データの整合性を保つため）
- メンバー対象: **コンサルタント（role='user'）のみを表示**（管理者は目標設定の対象外）
- 保存方法: **全メンバー分を一括保存**（個別保存だと操作が煩雑になるため）
- 未設定メンバーの扱い: **初期値（0）で表示し、編集可能にする**（新規メンバー追加時も対応可能）
