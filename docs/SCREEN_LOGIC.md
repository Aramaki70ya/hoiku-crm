...
**ステータス変更タイミングベースの追加表**: 既存の当月登録ベース表は変更せず、同じカード内に補助表として表示する。`担当`（担当求職者数）と`初回`（初回接触済み人数）は当月登録ベースと同じ集計を使い、`面接`（選択月内に初めて面接確定済・実施済へ到達した人数）と`成約`（選択月内に成約した人数）だけ、登録月を問わない月内実績ベースに差し替える。分母と分子の母集団が異なるため、説明文で補助表であることを明記する。
